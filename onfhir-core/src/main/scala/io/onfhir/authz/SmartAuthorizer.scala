...
class SmartAuthorizer extends IAuthorizer {
  ...
  private def resolveTargetAccessRight(scopes:Seq[WGHeartScope], resourceType:String, fhirInteraction:String):(Option[WGHeartScope], Option[WGHeartScope]) = {
    ...
    val mainScope =
      ...
      if(groupedScopes.get(PERMISSION_TYPE_PATIENT)
        .exists(_.exists(s => isAuthorizedForInteraction(s, fhirInteraction))) && fhirConfig.compartmentRelations.get("Patient").exists(_.isDefinedAt(resourceType))){
      ...
      else if(groupedScopes.get(PERMISSION_TYPE_USER).exists(_.exists(s => isAuthorizedForInteraction(s, fhirInteraction))) /*&& fhirConfig.compartmentRelations.filter(c => c._1 == "Practitioner" || c._1 == "RelatedPerson").exists(_._2.isDefinedAt(resourceType))*/){
